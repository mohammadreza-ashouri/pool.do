...
utils.$document.ready(function() {
                ...
                scales: {
                    xAxes: [{
                        ticks: {
                            fontSize: 18,
                            fontColor: "#fff",
                            callback: (val) => (new Date(val).toLocaleTimeString('en-GB', { hour12: false })) // or a different way to convert timestamp to date
                        },
                        display: true,
                        scaleLabel: {
                            display: false,
    ...
    // https://api2.chiaexplorer.com/chart/xchPriceChart?period=1d

    $.getJSON("https://api2.chiaexplorer.com/chart/xchPriceChart?period=24h", function(data) {
        var priceEl = document.getElementById('price');

        var dData = data.data.reverse();
        var timestamp = data.timestamp.reverse();

        priceEl.innerHTML = dData.pop()

        BuildChart(timestamp, dData, "Chia Price");
    });
